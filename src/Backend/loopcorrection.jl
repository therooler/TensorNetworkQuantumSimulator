...
## These functions are not used in the high level code. Just for clarity.
...
#     # partitioned_graph(pg) is the original unterlying TN graph
...
function enumerate_cycles(g::NamedGraph, max_cycle_length::Int64)
    ...
    # loops can look different but actually be equal due to order or directionality
...
function sim_src(bpc::BeliefPropagationCache, partition_edge::PartitionEdge)
    bpc = copy(bpc)
    old_ms = only(message(bpc, reverse(partition_edge)))
    linds = inds(old_ms)
    linds_sim = sim.(linds)
    old_ms = replaceinds(old_ms, inds(old_ms), linds_sim)
    ms = messages(bpc)
    set!(ms, reverse(partition_edge), ITensor[old_ms])
    verts = vertices(bpc, src(partition_edge))
    for v in verts
...
function sim_srcs(bpc::BeliefPropagationCache, partition_edges::Vector{<:PartitionEdge})
    bpc = copy(bpc)
    for partition_edge in partition_edges
        bpc = sim_src(bpc, partition_edge)
    end
    return bpc
end
...
function sim_circuit(bpc::BeliefPropagationCache, circuit)
    bpc = copy(bpc)
    involved_vertices = vertices_in_edgevector(circuit)
    for partition_vertex in involved_vertices
        bpc = sim_partitionvertex(bpc, partition_vertex)
    end
    return bpc
end
...
function circuit_weight(bpc::BeliefPropagationCache, circuit)
    # this preps the edges of the bpc for the anti-projectors to be inserted later
    bpc = sim_circuit(bpc, circuit)

    # gets the messages to the loop
...
function circuit_weights(bpc, circuits)
    return [circuit_weight(bpc, circuit) for circuit in circuits]
end
