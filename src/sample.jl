...
function StatsBase.sample(
    ...
    seq = [sorted_partitions[i] => sorted_partitions[i-1] for i in length(sorted_partitions):-1:2]
    ...
    for j in 1:nsamples
        ...
        for (i, partition) in enumerate(sorted_partitions)
            next_partition = i != length(sorted_partitions) ? sorted_partitions[i+1] : nothing
            ψIψ_j, p_over_q, bit_string, = sample_partition(ψIψ_j, partition, bit_string; kwargs...)
            vs = planargraph_vertices(ψIψ, partition)
            pψ = update_factors(pψ, Dict(zip(vs, [only(factors(ψIψ_j, [(v, "ket")])) for v in vs])))
            if !isnothing(next_partition)
                ms = messages(ψIψ_j)
                pψ = update(Algorithm("orthogonal"), pψ, partition => next_partition; left_message_update_kwargs...)
    ...
    bit_strings = [((p_over_q) / norm, bit_string) for (p_over_q, bit_string) in bit_strings]
    return bit_strings
end
...
function sample_partition(
    ...
    for v in vs
        ψIψ = !isnothing(prev_v) ? partition_update(ψIψ, [prev_v], [v]) : partition_update(ψIψ, [v])
